  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        if (p.flake) {
          ctx.strokeStyle = '#0369a1'; ctx.lineWidth = 0.6;
          ctx.translate(p.x, p.y);
          for (let i = 0; i < 6; i++) { ctx.rotate(Math.PI / 3); ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, p.r * 3.2); ctx.stroke(); }
        } else { ctx.fillStyle = '#0369a1'; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fill(); }
        ctx.restore();
        p.x += p.vx; p.y += p.vy;
  ...
  return (
    <div className={styles.thermo} aria-hidden="true">
      ...
      <div className={styles.thermoLabels}>
        <span className={styles.thermoTop}>Seamless</span>
        <span className={styles.thermoMid}>Waco, TX</span>
        <span className={styles.thermoBot}>Sectional</span>
      </div>
    </div>
  );
}

const CHIPS = ['Free Estimates', 'No Contracts', 'Factory-Certified', '13+ Yrs Local', 'Lifetime Seam Warranty'];
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.content}>
          <motion.div className={styles.badge}
            ...
            Waco&apos;s Most Trusted Gutter Company — Since 2013
          </motion.div>

          <motion.h1 className={styles.headline}
            initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.1 }}>
            Seamless Gutters.<br />Storm-Ready Guards.<br />
            <span className={styles.accentLine}>FlowGuard.</span>
          </motion.h1>

          <motion.p className={styles.sub}
            initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55, delay: 0.22 }}>
            Flat-rate pricing. Free on-site estimates. Lifetime Seam Warranty + 2-Year Labor.
          ...
          <motion.div className={styles.ctaRow}
            ...
            <a href="tel:+12547605500" className={styles.ctaPrimary}>
        ...
        >
          ...
          <motion.div className={`${styles.statCard} ${styles.sc1}`}
            ...
            <span className={styles.scNum}>6,000+</span>
          ...
          <motion.div className={`${styles.statCard} ${styles.sc3}`}
            ...
            <span className={styles.scNum}>Lifetime Seam</span>
            <span className={styles.scLbl}>Labor Warranty</span>
          </motion.div>
